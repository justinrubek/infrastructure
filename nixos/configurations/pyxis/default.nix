{
    ...
    secretFiles = {
      ...
      files = {
        pki = {
          ...
          perms = "0440";
          template = ''
            {{ with pkiCert "cluster/pki/etcd/issue/member" "common_name=etcd-server" "ip_sans=100.100.135.61,127.0.0.1" "ttl=420h" }}
            {{ .Key }}
            {{ .CA }}
            {{ .Cert }}
  ...
  justinrubek = {
    ...
    vault = {
      ...
      reader = {
        enable = true;
        secret_file = "${self}/secrets/nodes/controller1.yaml";
      };
  ...
  services = {
    etcd = {
      ...
      listenClientUrls = ["https://100.100.135.61:2379" "https://127.0.0.1:2379"];
      listenPeerUrls = ["https://100.100.135.61:2380"];
